...
let OrderController = class OrderController {
    async getAll(req, res, next) {
        try {
            const order = await services_1.OrderService.getAllOrders(req.query);
    ...
    async getOne(req, res, next) {
        try {
            const { id } = req.params;
            const order = await services_1.OrderService.getOneOrder(+id);
            return res.status(http_status_codes_1.StatusCodes.OK).json(order);
        }
        catch (error) {
            console.log(error);
    ...
    async update(req, res, next) {
        try {
            const { id } = req.params;
            const order = await services_1.OrderService.editOrder(+id, req.body);
            return res.status(http_status_codes_1.StatusCodes.OK).json(order);
        }
        catch (error) {
            console.log(error);
    ...
    async delete(req, res, next) {
        try {
            const { id } = req.params;
            await services_1.OrderService.deleteOrder(+id);
